...
(function(){
  ...
  var FIELD_STYLE={
    ...
    "交流":["","pink","community"],"相談":["","pink","community"],"居場所":["","pink","community"],"社会参画":["","pink","community"],
    "ボランティア":["","pink","community"],"国際交流":["","pink","community"],"チームビルディング":["","pink","community"],"若者支援":["","pink","community"],
    "自然体験":["","green","nature"],"キャンプ":["","green","nature"],"宿泊研修":["","green","nature"],"地域活動":["","green","nature"],"農業体験":["","green","nature"],
  ...
  var HUE_COLOR={
    green:"var(--color-accent)",blue:"var(--color-accent)",orange:"var(--color-accent)",
    purple:"var(--color-accent)",pink:"var(--color-accent)",teal:"var(--color-accent)"
  };
  ...
  function coverOf(r){
    ...
    for(var i=0;i<tags.length;i++){if(FIELD_STYLE[tags[i]])return {hue:FIELD_STYLE[tags[i]][1],catKey:FIELD_STYLE[tags[i]][2],label:tags[i]};}
    if(r.region==="全国オンライン")return {hue:"blue",catKey:"science",label:tags[0]||"オンライン"};
    var cats=r.cats||[];
    for(var j=0;j<cats.length;j++){if(CAT_FALLBACK[cats[j]])return {hue:CAT_FALLBACK[cats[j]][1],catKey:CAT_FALLBACK[cats[j]][2],label:tags[0]||cats[j]};}
    return {hue:"green",catKey:"nature",label:tags[0]||"活動"};
  }
  ...
  function markerIcon(cv){
    ...
    return L.divIcon({html:'<div class="ys-pin" style="background:'+color+'"></div>',className:"",iconSize:[32,32],iconAnchor:[16,16],popupAnchor:[0,-16]});
  }
  ...
  window.YS={
    ...
    FIELD_STYLE:FIELD_STYLE, CAT_KEYS:CAT_KEYS, CAT_LABELS:CAT_LABELS, CAT_FIELD_MAP:CAT_FIELD_MAP,
    esc:esc, el:el, coverOf:coverOf, mapUrl:mapUrl, markerIcon:markerIcon, card:card,
