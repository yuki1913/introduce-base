...
const require_array = require('../utils/array.cjs');
const require_with_retry = require('./with-retry.cjs');
...
var BaseClient = class {
	constructor(params) {
		this.fetch = require_with_retry.withRetry(params.fetch ?? globalThis.fetch);
	...
	async request(path, opts) {
		...
		if (opts?.query) for (const [key, value] of Object.entries(opts.query)) require_array.array(value).forEach((value$1) => {
...
async function parse(validator, response) {
	...
	const text = await response.text().catch((err) => {
		return new require_api_error.APIError(response, {
	...
	if (!response.ok) return new require_api_error.APIError(response, {
	...
	if (!validated.success) return new require_api_error.APIError(response, {
...
async function parseOrThrow(validator, response) {
	...
	if (result instanceof require_api_error.APIError) throw result;
	return result;
}
